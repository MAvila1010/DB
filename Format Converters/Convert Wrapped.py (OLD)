...
df = pd.read_excel("Old_320TC161120.xlsx")
...
df['LABEL_TYPECODE'] = df['Tile_Size'].apply(lambda x: f"TC-{str(x).split('.')[0].zfill(2)}")
...
df['BARCODE'] = '320TC' + df['Tile_Size'].apply(lambda x: str(x).split('.')[0].zfill(2)) + df['SERIAL_NUMBER']
df['BARCODE'] = df['BARCODE'].str.replace('-', '')  # Remove dashes from BARCODE column
df['LOCATION'] = 'NIU'
df['INSTITUTION'] = 'NIU'
df['MANUFACTURER'] = 'NIU'
df['NAME_LABEL'] = df.apply(lambda row: f"Wrap Cast Tile BH Ring {str(row['Tile_Size']).split('.')[0].zfill(2)} SN {row['SERIAL_NUMBER']}", axis=1)
...
df.to_excel("320TC161120.xlsx", index=False)
